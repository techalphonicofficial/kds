...
export default async function SolutionDetailPage({ params }) {
  try {
    ...
    const service = generateMockData(resolvedParams.slug, "solutions");
                  ...
                  <div id="content" className="scroll-mt-28 my-4 pt-3 border-t border-gray-200 dark:border-white/5">
                    {service.content.map((item, idx) => {
                      if (item.type === "heading") {
                        return (
                          <h3 key={idx} className="text-2xl font-black text-gray-900 dark:text-white mt-8 mb-4">
                            {item.text}
                          </h3>
                        );
                      } else if (item.type === "paragraph") {
                        return (
                          <p key={idx} className="text-gray-600 dark:text-[#8b949e] leading-relaxed mb-4">
                            {item.text}
                          </p>
                        );
